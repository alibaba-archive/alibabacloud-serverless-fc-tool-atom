...
class V8_EXPORT AllocationProfile {
 public:
  ...
  /**
   * Prepare a serialized representation of the allocation profile. The result
   * is written into the stream provided in chunks of specified size.
   *
   * For the JSON format, heap contents are represented as an object
   * with the following structure:
   ...
   *  }
   * 
   * The elements in the nodes array are of the same type as the head.
   */
  void Serialize(OutputStream* stream, SerializationFormat format, v8::Isolate* isolate) const;
};
