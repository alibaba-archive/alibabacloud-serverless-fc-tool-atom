...
export default class R {
  ...
  async checkPolicyNotExistOrEnsureAvailable(
    ...
    await retry(async (retry, times) => {
      try {
        ...
        this.logger.debug(
          `On-line policy config: ${JSON.stringify(onlinePolicyConfig)}`
        );
        const onlinePolicyDocument = JSON.parse(
          onlinePolicyConfig.DefaultPolicyVersion.PolicyDocument
        );
        this.logger.debug(
          `On-line default policy version document: ${JSON.stringify(
            onlinePolicyDocument
          )}`
        );
        ...
        if (
          !statement ||
          _.isEqual(onlinePolicyDocument.Statement, statement)
        ) {
          return;
        }
        ...
        this.logger.debug(
          `Error when getPolicy, policyName is ${policyName}, error is: ${ex}`
        );
        this.logger.info(`GetPolicy ${policyName}, retry ${times} time`);
        retry(ex);
      }
    }, RETRYOPTIONS);
  ...
  async checkRoleNotExistOrEnsureAvailable(
    ...
    try {
      ...
      this.logger.debug(
        `Get role ${roleName} response: ${JSON.stringify(roleResponse)}`
      );
      ...
      if (
        roleDocument &&
        JSON.stringify(roleDocument) !== AssumeRolePolicyDocument
      ) {
        this.logger.info(
          `${roleName} authorization policy is inconsistent with online.`
        );
  ...
  async createPolicy(policyName: string, statement: any, description?: string) {
    ...
    await retry(async (retry, times) => {
      try {
        ...
        this.logger.debug(
          `Error when createPolicy, policyName is ${policyName}, error is: ${ex}`
        );
        this.logger.info(`CreatePolicy ${policyName}, retry ${times} time`);
        retry(ex);
      }
    }, RETRYOPTIONS);
  ...
  async createRole(
    ...
    try {
      ...
      this.logger.debug(
        `Error when createRole, roleName is ${name}, error is: ${ex}`
      );
      throw ex;
    }
  }
  ...
  async updatePolicy(policyName: string, statement: any) {
    ...
    await retry(async (retry, times) => {
      try {
        ...
        this.logger.debug(
          `Policy listPolicyVersions response: ${JSON.stringify(listResponse)}`
        );

        const versions =
          (listResponse.PolicyVersions || {}).PolicyVersion || [];
        ...
        this.logger.debug(
          `Error when updatePolicy, policyName is ${policyName}, error is: ${ex}`
        );
        this.logger.info(`updatePolicy ${policyName}, retry ${times} time`);
        retry(ex);
      }
    }, RETRYOPTIONS);
  ...
  async updateRole(name: string, roleDocument: IRoleDocument) {
    try {
      ...
      this.logger.debug(
        `Error when updateRole, roleName is ${name}, error is: ${ex}`
      );
      throw ex;
    }
  }
  ...
  async mackPlicys(policys: Array<string | IPolicy>): Promise<IPolicyName[]> {
    ...
    for (const policy of policys) {
      if (_.isString(policy)) {
        ...
        policyNameAvailable = await this.checkPolicyNotExistOrEnsureAvailable(
          policyName,
          'Custom'
        );
        if (!policyNameAvailable) {
  ...
  async attachPolicysToRole(policyNamesArray: IPolicyName[], roleName: string) {
    ...
    await retry(async (retry, times) => {
      try {
        ...
        this.logger.debug(
          `Get list policies for ${roleName} response: ${JSON.stringify(
            policies
          )}`
        );
      } catch (ex) {
        ...
        this.logger.info(
          `AttachPolicysToRole ${roleName}, retry ${times} time`
        );
        retry(ex);
      }
    }, RETRYOPTIONS);
    ...
    for (const { name, type } of policyNamesArray) {
      await retry(async (retry, times) => {
        ...
        try {
          ...
          if (policy) {
            this.logger.info(
              `Policy(${name}) already exists in ${roleName}, skip attach.`
            );
          } else {
          ...
          this.logger.info(`Attach policy(${name}), retry ${times} time`);
          retry(ex);
        }
      }, RETRYOPTIONS);
    }
    ...
    for (const item of policys) {
      ...
      await retry(async (retry, times) => {
        try {
          ...
          this.logger.debug(
            `Policy listPolicyVersions response: ${JSON.stringify(
              listPolicyVersionResponse
            )}`
          );
          const versions =
            (listPolicyVersionResponse.PolicyVersions || {}).PolicyVersion ||
            [];
          ...
          this.logger.info(`Delete policy ${policyName}, retry ${times} time`);
          retry(ex);
        }
      }, RETRYOPTIONS);
    }
  ...
  async deleteRole(roleName: string) {
    ...
    await retry(async (retry, times) => {
      try {
        ...
        this.logger.debug(
          `Error when deleteRole, roleName is ${roleName}, error is: ${ex}`
        );
        this.logger.info(`Delete role ${roleName}, retry ${times} time`);
        retry(ex);
      }
    }, RETRYOPTIONS);
  }
